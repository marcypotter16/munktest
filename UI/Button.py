...
class TextButton(UIElement):
    def __init__(self, parent: UICanvas = None, x=0, y=0, center=None, width=100, height=100,
                 ...
                 fg_color=(0, 0, 0), text: str = "", corner_radius=10, command=lambda: print("Clicked"), hover_color=(150, 150, 150)):
...
class ImageButton(TextButton):
    def __init__(self, parent: UICanvas = None, x=0, y=0, center=None, width=100, height=100,
                 ...
                 hover_color=(150, 150, 150), animation: list[pygame.image] = None, animation_fps: int = 60):
        super().__init__(parent, x, y, center, width, height, bg_color, fg_color, text, corner_radius, command, hover_color)
        self.animation: list[pygame.image] = [pygame.transform.scale(image, self.rect.size) for image in animation]
        self.current_image_index: int = 0
        self.current_image: pygame.image = self.animation[0]
        self.animation_list_length: int = len(animation)
        self.prev_timestamp: float = 0
        self._MS_BETWEEN_ANIMATION_FRAMES: float = float(self.game.settings['fps']) / animation_fps
    ...
    def unhover(self):
        """
        Resets the animation and color
        :return:
        """
    ...
    def render(self, surface: pygame.Surface):
        if self.visible:
            super().render(surface)
            surface.blit(self.animation[self.current_image_index], self.rect)
